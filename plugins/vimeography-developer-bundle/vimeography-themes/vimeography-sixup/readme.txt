...
Sixup displays the 6 latest video thumbnails from your Vimeo collection, which can be clicked for navigating the slider.
...
1. Upload `vimeography-sixup.zip` to the `/wp-content/plugins/` directory
...
= 2.0 =
* Rewrote the 6up theme for Vimeography 2.0 compatibility

= 1.2.3 =
* Theme is now loaded as soon as the plugin class is instantiated.

= 1.2.2 =
* This update helps make sure your site doesn't run into errors if the Vimeography plugin is deactivated.

= 1.2.1 =
* Load a local copy of Vimeo's Froogaloop library
...
= 1.1 =
* Changed the assets path
* Added support for playlisting with Vimeography Pro
