...
class WPF_Woocommerce extends WPF_Integrations_Base {
	...
	public function handle_bulk_actions( $redirect_to, $action, $ids ) {
		...
		$max_items = apply_filters( 'wpf_bulk_order_actions_max_orders', 20 );
		...
		if ( count( $ids ) > $max_items ) {
			...
			wpf_log( 'notice', 0, 'Bulk order status change detected for ' . count( $ids ) . ' orders. The maxinum number of orders that can be processed at once is 20. To avoid a timeout, no data will be synced to ' . wp_fusion()->crm->name . '. You can re-sync the orders by running a batch operation from the WP Fusion Advanced settings tab.' );
		}
