...
$html         = $htmlDetail = $write_comments = $metaHtml = $iTitle = $catHtml = null;
...
if ( in_array( 'title', $items ) ) {
	if ( ! empty( $metaHtml ) && $metaPosition == 'above_title' ) {
		$iTitle .= "<div class='post-meta-user {$metaSeparator}'>$metaHtml</div>";
	}

	if ( $category_position == 'above_title' ) {
		$iTitle .= sprintf( '<div class="cat-above-title %s">%s</div>', $category_style, $catHtml );
	}
...
if ( ! empty( $metaHtml ) && empty( $metaPosition ) ) {
...
if ( ! empty( $metaHtml ) && $metaPosition == 'above_excerpt' ) {
	$html .= "<div class='post-meta-user {$metaSeparator}'>$metaHtml</div>";
}
