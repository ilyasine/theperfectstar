...
$html         = $titleHtml = $metaHtml = $contentHtml = $catHtml = null;
...
if ( ! empty( $metaHtml ) && $metaPosition == 'above_title' ) {
	$titleHtml .= "<p class='post-meta-user {$metaSeparator}'>$metaHtml</p>";
}
...
if ( ! empty( $metaHtml ) && ( empty( $metaPosition ) || $metaPosition == 'above_excerpt' ) ) {
	$contentHtml .= "<p class='post-meta-user {$metaSeparator}'>$metaHtml</p>";
}
