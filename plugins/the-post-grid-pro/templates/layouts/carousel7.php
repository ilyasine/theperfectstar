...
$html         = $metaHtml = $titleHtml = $catHtml = null;
...
if ( ! empty( $metaHtml ) && ( empty( $metaPosition ) || $metaPosition == 'above_excerpt' ) ) {
	$excerptContent .= "<div class='post-meta-user {$metaSeparator}'>$metaHtml</div>";
}
...
if ( in_array( 'cf', $items ) && ! empty( $cf_group ) ) {
	$excerptContent .= Functions::get_cf_formatted_fields( $cf_group, $format, $pID );
}
...
$html .= "<div class='post-info'>{$titleHtml}{$excerptContent}{$bottomContent}</div>";
$html .= '</div>';
$html .= '</div>';
$html .= '</div>';
