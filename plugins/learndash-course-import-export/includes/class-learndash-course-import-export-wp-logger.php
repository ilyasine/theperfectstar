...
class LearnDash_Course_Import_Export_WP_Logging {
	...
	function __construct() {
		...
		add_action( 'ldcie_wp_logging_prune_routine', array( $this, 'prune_logs' ) );
	 ...
	 *
	 * @filter ldcie_wp_logging_force_delete_log         Allows user to override the force delete setting which bypasses the trash
	 */
	private function prune_old_logs( $logs ){

		$force = apply_filters( 'ldcie_wp_logging_force_delete_log', true );
	 ...
	 *
	 * @filter ldcie_wp_logging_prune_when           Users can change how long ago we are looking for logs to prune
	 * @filter ldcie_wp_logging_prune_query_args     Gives users access to change any query args for pruning
	 */
	private function get_logs_to_prune(){

		$how_old = apply_filters( 'ldcie_wp_logging_prune_when', '-1 week' );

		$args = array(
			'post_type'      => 'ldcie_wp_log',
			'posts_per_page' => '100',
			'date_query'     => array(
				array(
		...
		$old_logs = get_posts( apply_filters( 'ldcie_wp_logging_prune_query_args', $args ) );
		return $old_logs;	
	...
	*/
	...
	private static function log_types() {
		...
		return apply_filters( 'ldcie_wp_log_types', $terms );
	}


	/**
	 * Registers the ldcie_wp_log Post Type
	 *
	 * @access      public
	 * @since       1.0
	 *
	...
	*/
	...
	public function register_post_type() {
		...
		register_post_type( 'ldcie_wp_log', apply_filters( 'ldcie_wp_logging_post_type_args', $log_args ) );
	...
	*/
	...
	public function register_taxonomy() {
		...
		register_taxonomy( 'ldcie_wp_log_type', 'ldcie_wp_log', array( 'public' => defined( 'WP_DEBUG' ) && WP_DEBUG ) );

		$types = self::log_types();

		foreach ( $types as $type ) {
			if( ! term_exists( $type, 'ldcie_wp_log_type' ) ) {
				wp_insert_term( $type, 'ldcie_wp_log_type' );
			}
		}
	}
	...
	*/
	...
	public static function insert_log( $log_data = array(), $log_meta = array() ) {
		...
		if( $log_data['log_type'] && self::valid_type( $log_data['log_type'] ) ) {
			wp_set_object_terms( $log_id, $log_data['log_type'], 'ldcie_wp_log_type', false );
		}
		...
		if( $log_id && ! empty( $log_meta ) ) {
			foreach( (array) $log_meta as $key => $meta ) {
				update_post_meta( $log_id, '_ldcie_wp_log_' . sanitize_key( $key ), $meta );
			}
		}
...
new LearnDash_Course_Import_Export_WP_Logging();