		...
		if ( ! empty( $predefined_packages ) ) {
			...
			if ( ! empty( $duplicate_package_names_by_carrier ) ) {
				$error = array(
					...
					'message' => __( 'The new predefined package names are not unique.', 'woocommerce-shipping' ),
			...
			if ( ! empty( $duplicate_package_names_by_carrier ) ) {
				$error = array(
					...
					'message' => __( 'At least one of the new predefined packages has the same name as existing packages.', 'woocommerce-shipping' ),
					'data'    => array( 'package_names_by_carrier' => $duplicate_package_names_by_carrier ),
				);
				return new WP_REST_Response( $error, 400 );
			}
