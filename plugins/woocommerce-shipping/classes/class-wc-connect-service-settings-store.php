			...
			if ( $label_data['label_id'] === $new_label_data->label_id ) {
				...
				if ( ! isset( $label_data['tracking'] )
					&& isset( $result['tracking'] ) ) {
						// We use the service name as the carrier id.
						$carrier_id = $result['service_name'];
						WC_Connect_Extension_Compatibility::on_new_tracking_number( $order_id, $carrier_id, $result['tracking'] );
				}
			}
		}
