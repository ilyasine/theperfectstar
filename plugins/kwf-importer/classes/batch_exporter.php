...
class TPRM_importer_Batch_Exporter{
	protected $path = '';
    ...
    function __construct() {
        add_filter( 'TPRM_importer_export_columns', array( $this, 'maybe_order_columns_alphabetacally' ), 10, 2 );
        add_filter( 'TPRM_importer_export_columns', array( $this, 'maybe_order_columns_filtered_columns_parameter' ), 11, 2 );
		add_filter( 'TPRM_importer_export_data', array( $this, 'maybe_double_encapsulate_serialized_values' ), 8, 3 );
        add_filter( 'TPRM_importer_export_data', array( $this, 'maybe_order_row_alphabetically' ), 10, 3 );
        add_filter( 'TPRM_importer_export_data', array( $this, 'maybe_order_row_filtered_columns_parameter' ), 11, 5 );
    ...
    function get_non_date_keys(){
		return apply_filters( 'TPRM_importer_export_non_date_keys', array_merge( $this->user_data, $this->woocommerce_default_user_meta_keys, $this->other_non_date_keys ) );
	}
    ...
    function load_columns(){
		...
		$this->set_columns_to_export( apply_filters( 'TPRM_importer_export_columns', $row, array( 'order_fields_alphabetically' => $this->get_order_fields_alphabetically(), 'double_encapsulate_serialized_values' => $this->get_double_encapsulate_serialized_values(), 'filtered_columns' => $this->get_filtered_columns() ) ) );
    }
    ...
    function get_total_rows(){
        $total_rows = get_transient( 'TPRM_importer_export_total_rows' );
	...
	function export() {
		...
		do_action( 'TPRM_importer_export_before_delete_file', $this->get_file_path() );
    ...
    function calculate_total(){
        ...
        set_transient( 'TPRM_importer_export_total_rows', $total_rows, HOUR_IN_SECONDS );
	...
	function prepare_data_to_export() {
		...
		foreach ( $users as $user ) {
            ...
            if( count( $this->get_filtered_columns() ) == 0 || in_array( 'role', $this->get_filtered_columns() ) )
			    $row['role'] = implode( ',', $TPRM_importer_helper->get_roles_by_user_id( $user ) );
    ...
    function prepare( $key, $value, $datetime_format = '', $user = 0 ){
		...
		$timestamp_keys = apply_filters( 'TPRM_importer_export_timestamp_keys', array( 'wc_last_active' ) );
		$original_value = $value;
		$value = $this->clean_bad_characters_formulas( $value, $key, $user );

        if( has_filter( 'TPRM_importer_export_prepare' ) ){
            return apply_filters( 'TPRM_importer_export_prepare', $value, $original_value );
        }

		if( $key == 'role' ){
			return implode( ',', $TPRM_importer_helper->get_roles_by_user_id( $user ) );
		}
	...
	function add_bad_character_formulas_values_cleaned( $key, $value, $user ){
		$current = get_transient( 'TPRM_importer_export_bad_character_formulas_values_cleaned' );
		if( !is_array( $current ) )
			$current = array();

		$current[] = array( 'key' => $key, 'value' => $value, 'user_id' => $user );
		set_transient( 'TPRM_importer_export_bad_character_formulas_values_cleaned', $current );
	}
    ...
    function get_user_meta_keys() {
        ...
        $usermeta = get_transient( 'TPRM_importer_export_user_meta_keys' );
		$usermeta = '';

        if( empty( $usermeta ) ){
            $usermeta = $wpdb->get_results( "SELECT distinct $wpdb->usermeta.meta_key FROM $wpdb->usermeta", ARRAY_A );
            set_transient( 'TPRM_importer_export_user_meta_keys', $usermeta, HOUR_IN_SECONDS );
        }
	    ...
	    return apply_filters( 'TPRM_importer_export_get_user_meta_keys', $meta_keys );
	}
}
