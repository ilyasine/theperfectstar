...
class TPRM_importer_Helper{
    ...
    static function get_editable_roles( $include_no_role = true ){
        ...
        if( $include_no_role )
            $list_editable_roles['no_role'] = __( 'No role', 'kwf-importer' );
    ...
    static function get_csv_delimiters_titles(){
        return array(
            'COMMA' => __( 'Comma', 'kwf-importer' ),
            'COLON' => __( 'Colon', 'kwf-importer' ),
            'SEMICOLON' => __( 'Semicolon', 'kwf-importer' ),
            'TAB' => __( 'Tab', 'kwf-importer' ),
        );
    }
    ...
    function get_restricted_fields(){
        ...
        $TPRM_importer_restricted_fields = array_merge( $wp_users_fields, $wp_min_fields );
        
        return apply_filters( 'TPRM_importer_restricted_fields', $TPRM_importer_restricted_fields );
    }

    function get_not_meta_fields(){
        return apply_filters( 'TPRM_importer_not_meta_fields', array() );
    }
    ...
    function print_table_header_footer( $headers ){
        ?>
        <h3><?php echo apply_filters( 'TPRM_importer_log_inserting_updating_data_title', __( 'Inserting and updating data', 'kwf-importer' ) ); ?></h3>
        <table id="TPRM_importer_results">
            <thead>
                <tr>
                    <th><?php _e( 'Row', 'kwf-importer' ); ?></th>
                    <?php foreach( $headers as $element ): 
                        echo "<th>" . esc_html( $element ) . "</th>"; 
                    endforeach; ?>
                    <?php do_action( 'TPRM_importer_header_table_extra_rows' ); ?>
                </tr>
            </thead>
            <tfoot>
                <tr>
    ...
    function print_row_imported( $row, $data, $errors ){
        ...
        if( !empty( TPRM_importer_Helper::get_errors_by_row( $errors, $row, 'any' ) ) )
    ...
    function print_errors( $errors ){
        if( empty( $errors ) )
            return;
        ?>
        <h3><?php _e( 'Errors, warnings and notices', 'kwf-importer' ); ?></h3>
        <table id="TPRM_importer_errors">
            <thead>
                <tr>
                    <th><?php _e( 'Row', 'kwf-importer' ); ?></th>
                    <th><?php _e( 'Details', 'kwf-importer' ); ?></th>
                    <th><?php _e( 'Type', 'kwf-importer' ); ?></th>
                </tr>
            </thead>
            <tfoot>
                <tr>
    ...
    function print_results( $results, $errors ){
        ?>
        <h3><?php _e( 'Results', 'kwf-importer' ); ?></h3>
        <table id="TPRM_importer_results">
            <tbody>
                <tr>
                    <th><?php _e( 'Users processed', 'kwf-importer' ); ?></th>
                    <td><?php echo $results['created'] + $results['updated']; ?></td>
                </tr>
                <tr>
                    <th><?php _e( 'Users created', 'kwf-importer' ); ?></th>
                    <td><?php echo $results['created']; ?></td>
                </tr>
                <tr>
                    <th><?php _e( 'Users updated', 'kwf-importer' ); ?></th>
                    <td><?php echo $results['updated']; ?></td>
                </tr>
                <tr>
                    <th><?php _e( 'Users deleted', 'kwf-importer' ); ?></th>
                    <td><?php echo $results['deleted']; ?></td>
                </tr>
                <tr>
                    <th><?php _e( 'Errors, warnings and notices found', 'kwf-importer' ); ?></td>
    ...
    function class_print_results( $results, $ecole_results, $errors ){
        ?>
        <h3><?php _e( 'School & Classes', 'kwf-importer' ); ?></h3>
        <table id="TPRM_importer_results">
            <tbody>
                <tr>
                    <th><?php _e( 'School', 'kwf-importer' ); ?></th>
                    <td><?php echo $ecole_results['created'] + $ecole_results['updated']; ?></td>
                </tr>
                <tr>
                    <th><?php _e( 'Classes processed', 'kwf-importer' ); ?></th>
                    <td><?php echo $results['created'] + $results['updated']; ?></td>
                </tr>
                <tr>
                    <th><?php _e( 'Classes created', 'kwf-importer' ); ?></th>
                    <td><?php echo $results['created']; ?></td>
                </tr>
                <tr>
                    <th><?php _e( 'Classes updated', 'kwf-importer' ); ?></th>
                    <td><?php echo $results['updated']; ?></td>
                </tr>
                <tr>
                    <th><?php _e( 'Classes deleted', 'kwf-importer' ); ?></th>
                    <td><?php echo $results['deleted']; ?></td>
                </tr>
                <tr>
                    <th><?php _e( 'Errors, warnings and notices found', 'kwf-importer' ); ?></td>
    ...
    function print_end_of_process(){
        ?>
        <br/>
        <p><?php printf( __( 'Process finished you can go <a href="%s">here to see results</a> or you can do <a href="%s">a new import</a>.', 'kwf-importer' ), get_admin_url( null, 'users.php' ), get_admin_url( null, 'admin.php?page=TPRM_importer&tab=import-users' ) ); ?></p>
        <?php
    }

    function print_end_of_group_process(){
        ?>
        <br/>
        <p><?php printf( __( 'Process finished you can go <a href="%s">here to see results</a> or you can do <a href="%s">a new import</a>.', 'kwf-importer' ), get_admin_url( null, 'admin.php?page=bp-groups' ), get_admin_url( null, 'admin.php?page=TPRM_importer&tab=import-groups' ) ); ?></p>
        <?php
    }
    ...
    function execute_datatable(){
        ...
        jQuery( document ).ready( function( $ ){
            $( '#TPRM_importer_results,#TPRM_importer_errors' ).DataTable({
                "scrollX": true,
            });
        } )
        </script>
    ...
    function basic_css(){
        ...
        <style type="text/css">
            ...
            #TPRM_importer_results_wrapper .dataTables_scroll, #TPRM_importer_errors_wrapper .dataTables_scrollBody{
    ...
    static function get_notices(){
        $notices = get_transient( 'TPRM_importer_notices' );
        delete_transient( 'TPRM_importer_notices' );
    ...
    static function add_notice( $notice ){
        ...
        $notices[] = $notice;
        set_transient( 'TPRM_importer_notices', $notices, 120 );
    }
    ...
    function get_notice(){
        ...
        foreach( $notices as $key => $notice ){
            $return = $notice;
            unset( $notices[ $key ] );
            set_transient( 'TPRM_importer_notices', $notices );
            return $return;
        }
