...
class TPRM_importer_HTML{
    ...
    private static function is_instantiated() {
		if ( ! empty( self::$instance ) && ( self::$instance instanceof TPRM_importer_HTML ) ) {
			return true;
		}
    ...
    private static function setup_instance() {
		self::$instance = new TPRM_importer_HTML;
	}
	...
	function select( $args = array() ) {
		$defaults = array(
			...
			'placeholder'      => null,
			'multiple'         => false,
			'show_option_all'  => _x( 'All', 'all dropdown items', 'kwf-importer' ),
			'show_option_none' => _x( 'None', 'no dropdown items', 'kwf-importer' ),
			'data'             => array(),
			'readonly'         => false,
			'disabled'         => false,
		);
		...
		$output = '<select' . $disabled . $readonly . ' name="' . esc_attr( $args['name'] ) . '" id="' . esc_attr( $this->sanitize_key( $args['id'] ) ) . '" class="TPRM_importer-select ' . $class . '"' . $multiple . ' data-placeholder="' . $placeholder . '"'. $data_elements . '>';
	...
	function checkbox( $args = array() ) {
		...
		$class = 'TPRM_importer-checkbox ' . implode( ' ', array_map( 'sanitize_html_class', explode( ' ', $args['class'] ) ) );
		$options = '';
	...
	function text( $args = array() ) {
		...
		$output = '<span id="TPRM_importer-' . $this->sanitize_key( $args['name'] ) . '-wrap">';
			if ( ! empty( $args['label'] ) ) {
				$output .= '<label class="TPRM_importer-label" for="' . $this->sanitize_key( $args['id'] ) . '">' . esc_html( $args['label'] ) . '</label>';
			}

			if ( ! empty( $args['desc'] ) ) {
				$output .= '<span class="TPRM_importer-description">' . esc_html( $args['desc'] ) . '</span>';
			}
	...
	function textarea( $args = array() ) {
		...
		$output = '<span id="TPRM_importer-' . $this->sanitize_key( $args['name'] ) . '-wrap">';
			...
			if ( ! empty( $args['label'] ) ) {
				$output .= '<label class="TPRM_importer-label" for="' . $this->sanitize_key( $args['name'] ) . '">' . esc_html( $args['label'] ) . '</label>';
			}
...
function TPRM_importerHTML(){
    return TPRM_importer_HTML::instance();
}