...
class TPRM_importer_Settings{
    ...
    function __construct( $current_context = '' ){
        $this->settings = apply_filters( 'TPRM_importer_settings', array(
            'import_common' => array(
                'path_to_file' => array( 
                    'sanitization' => 'text',
                    'default' => ''
        ...
        $this->key = 'TPRM_importer_settings';
        $this->contexts = apply_filters( 'TPRM_importer_contexts', array( 'import_backend', 'import_frontend', 'import_cron', 'export_backend') );
    ...
    private static function is_instantiated() {
		if ( ! empty( self::$instance ) && ( self::$instance instanceof TPRM_importer_Settings ) ) {
			return true;
		}
    ...
    private static function setup_instance() {
		self::$instance = new TPRM_importer_Settings;
	}
    ...
    function maybe_migrate_old_options(){
        switch( $this->current_context ){
            case 'import_backend':
                if ( get_option( 'TPRM_importer_last_roles_used', 'not-exists' ) === 'not-exists' ){
                    return;
                }                    

		        $last_roles_used = empty( get_option( 'TPRM_importer_last_roles_used' ) ) ? array( 'subscriber' ) : get_option( 'TPRM_importer_last_roles_used' );
                delete_option( 'TPRM_importer_last_roles_used' );
                $this->save( 'role', $last_roles_used );

                $path_to_file = empty( get_option( 'TPRM_importer_path_to_file' ) ) ? dirname( __FILE__ ) . '/test.csv' : get_option( 'TPRM_importer_path_to_file' );
                delete_option( 'TPRM_importer_path_to_file' );
                $this->save( 'path_to_file', $path_to_file );

                $manually_send_mail =  get_option( 'TPRM_importer_manually_send_mail' ) ? 'yes' : 'no';
                delete_option( 'TPRM_importer_manually_send_mail' );
                $this->save( 'sends_email', $manually_send_mail );

                $manually_send_mail_updated =  get_option( 'TPRM_importer_manually_send_mail_updated' ) ? 'yes' : 'no';
                delete_option( 'TPRM_importer_manually_send_mail_updated' );
                $this->save( 'send_email_updated', $manually_send_mail_updated );

                $manually_force_user_reset_password =  get_option( 'TPRM_importer_manually_force_user_reset_password' ) ? 'yes' : 'no';
                delete_option( 'TPRM_importer_manually_force_user_reset_password' );
...
function TPRM_importerSettings(){
    return TPRM_importer_Settings::instance();
}