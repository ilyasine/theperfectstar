...
class TPRM_importer_WooCommerceSubscriptions{
	private $all_virtual;

	function __construct(){
		/* add_filter( 'TPRM_importer_restricted_fields', array( $this, 'restricted_fields' ), 10, 1 );
		add_action( 'TPRM_importer_header_table_extra_rows', array( $this, 'header_table_extra_rows' ) );
		add_action( 'TPRM_importer_documentation_after_plugins_activated', array( $this, 'documentation' ) );
		add_action( 'after_TPRM_importer_import_users', array( $this, 'end' ) );		
	...
	function header_table_extra_rows(){
		?>
		<th><?php _e( "WooCommerce Subscriptions import warnings", 'kwf-importer' ); ?></th>
		<th><?php _e( "WooCommerce Subscriptions import errors", 'kwf-importer' ); ?></th>
		<?php
	}
	...
	?>
		<tr valign="top">
			<th scope="row"><?php _e( "WooCommerce Subscriptions is activated", 'kwf-importer' ); ?></th>
			<td>
				<ol>
					<li><?php _e( 'You can import subscriptions with their info', 'kwf-importer' ); ?>.</li>
					<li><?php printf( __( 'Data format: use the column names <a href="%s">described here</a>, except the ones that are not used because we have the own ones:', 'kwf-importer' ), "https://github.com/woocommerce/woocommerce-subscriptions-importer-exporter#csv-columns" ); ?> customer_id, customer_email and customer_username.</li>
					<li><?php printf( __( 'Download <a href="%s">this sample file</a> to see how it works', 'kwf-importer' ), esc_url( plugins_url( 'samples/wcs-import-sample.csv', dirname( __FILE__ ) ) ) ); ?>.</li>
					<li><?php printf( __( 'This subscription importer is based in the official <a href="%s">WooCommerce Subscription Importer Exporter</a>.', 'kwf-importer' ), "https://github.com/woocommerce/woocommerce-subscriptions-importer-exporter" ); ?></li>
				</ol>
			</td>
		</tr>
		<?php
	...
	function import( $headers, $row, $user_id ){
		...
		if ( empty( $data[ 'subscription_status'] ) ) {
			$status              = 'pending';
			$result['warning'][] = esc_html__( 'No subscription status was specified. The subscription will be created with the status "pending". ', 'kwf-importer' );
		} else {
			...
			switch ( $date_type ) {
				case 'end_date' :
					if ( ! empty( $dates_to_update['next_payment_date'] ) && strtotime( $datetime ) <= strtotime( $dates_to_update['next_payment_date'] ) ) {
						$result['error'][] = sprintf( __( 'The %s date must occur after the next payment date.', 'kwf-importer' ), $date_type );
					}
				case 'next_payment_date' :
					if ( ! empty( $dates_to_update['trial_end_date'] ) && strtotime( $datetime ) < strtotime( $dates_to_update['trial_end_date'] ) ) {
						$result['error'][] = sprintf( __( 'The %s date must occur after the trial end date.', 'kwf-importer' ), $date_type );
					}
				case 'trial_end_date' :
					if ( strtotime( $datetime ) <= strtotime( $dates_to_update['start'] ) ) {
						$result['error'][] = sprintf( __( 'The %s must occur after the start date.', 'kwf-importer' ), $date_type );
					}
			}
		}
		...
		if ( 'pending-cancel' == $status && ( empty( $dates_to_update['next_payment_date'] ) || strtotime( $dates_to_update['next_payment_date'] ) < current_time( 'timestamp', true ) ) ) {
			if ( ! empty( $dates_to_update['end_date'] ) && strtotime( $dates_to_update['end_date'] ) > current_time( 'timestamp', true ) ) {
				...
				$result['error'][] = __( 'Importing a pending cancelled subscription requires an end date in the future.', 'kwf-importer' );
			}
		}
		...
		if ( empty( $result['error'] ) ) {
			try {
				...
				if ( is_wp_error( $subscription ) ) {
					throw new Exception( sprintf( esc_html__( 'Could not create subscription: %s', 'kwf-importer' ), $subscription->get_error_message() ) );
				}
				...
				if ( $set_manual ) {
					$result['warning'][] = esc_html__( 'No payment method was given in CSV and so the subscription has been set to manual renewal.', 'kwf-importer' );
				} else if ( $requires_manual_renewal ) {
					$result['warning'][] = esc_html__( 'Import forced manual renewal.', 'kwf-importer' );
				}
				...
				if ( ! $this->all_virtual ) {
					if ( ! empty( $missing_shipping_addresses ) ) {
						$result['warning'][] = sprintf( esc_html__( 'The following shipping address fields have been left empty: %s. ', 'kwf-importer' ), rtrim( implode( ', ', $missing_shipping_addresses ), ',' ) );
					}

					if ( ! empty( $missing_billing_addresses ) ) {
						$result['warning'][] = sprintf( esc_html__( 'The following billing address fields have been left empty: %s. ', 'kwf-importer' ), rtrim( implode( ', ', $missing_billing_addresses ), ',' ) );
					}

					if ( empty( $shipping_method ) ) {
						$result['warning'][] = esc_html__( 'Shipping method and title for the subscription have been left as empty. ', 'kwf-importer' );
					}
				}
		...
		if ( ! empty( $payment_method ) ) {
			...
			if ( ! empty( $payment_gateway ) && isset( $payment_method_table[ $payment_gateway->id ] ) ) {
				...
				if ( $meta_set ) {
					...
					$warnings[] = sprintf( esc_html__( 'No payment meta was set for your %1$s subscription (%2$s). The next renewal is going to fail if you leave this.', 'kwf-importer' ), $payment_method, $subscription_id );
				}
			} else {
				if ( 'paypal' == $payment_method ) {
					$warnings[] = sprintf( esc_html__( 'Could not set payment method as PayPal, defaulted to manual renewals. Either PayPal was not enabled or your PayPal account does not have Reference Transaction setup. Learn more about enabling Reference Transactions %1$shere%2$s.', 'kwf-importer' ), '<a href="https://support.woocommerce.com/hc/en-us/articles/205151193-PayPal-Reference-Transactions-for-Subscriptions">', '</a>' );
				} else {
					$warnings[] = sprintf( esc_html__( 'The payment method "%s" is either not enabled or does not support the new features of Subscriptions 2.0 and can not be properly attached to your subscription. This subscription has been set to manual renewals.', 'kwf-importer' ), $payment_method );
				}
				$subscription->set_requires_manual_renewal( true );
			}
		}
	...
	function add_coupons( $subscription, $data ) {
		...
		if ( ! empty( $coupon_items ) ) {
			foreach ( $coupon_items as $coupon_item ) {
				...
				if ( ! $coupon ) {
					throw new Exception( sprintf( esc_html__( 'Could not find coupon with code "%s" in your store.', 'kwf-importer' ), $coupon_code ) );
				...
				if ( ! $coupon_id ) {
					throw new Exception( sprintf( esc_html__( 'Coupon "%s" could not be added to subscription.', 'kwf-importer' ), $coupon_code ) );
				}
			}
		}
	}
	...
	function add_product( $subscription, $data, $chosen_tax_rate_id ) {
		...
		if ( ! isset( $data['product_id'] ) ) {
			throw new Exception( __( 'The product_id is missing from CSV.', 'kwf-importer' ) );
		}

		$_product = wc_get_product( $data['product_id'] );

		if ( ! $_product ) {
			throw new Exception( sprintf( __( 'No product or variation in your store matches the product ID #%s.', 'kwf-importer' ), $data['product_id'] ) );
		}
		...
		if ( ! $item_id ) {
			throw new Exception( __( 'An unexpected error occurred when trying to add product "%s" to your subscription. The error was caught and no subscription for this row will be created. Please fix up the data from your CSV and try again.', 'kwf-importer' ) );
		}
		...
		if ( ! empty( $fee_items ) ) {
			foreach ( $fee_items as $fee_item ) {
				...
				if ( empty( $fee_data['name'] ) ) {
					throw new Exception( __( 'Fee name is missing from your CSV. This subscription has not been imported.', 'kwf-importer' ) );
				}
				...
				if ( ! $fee_id ) {
					throw new Exception( __( 'Could not add the fee to your subscription, the subscription has not been imported.', 'kwf-importer' ) );
				}
			}
		}
	}
	...
	function add_shipping_lines( $subscription, $data, $chosen_tax_rate_id ) {
		...
		if ( ! empty( $shipping_items ) ) {
			foreach ( $shipping_items as $shipping_item ) {
				...
				if ( ! $shipping_id ) {
					throw new Exception( __( 'An error occurred when trying to add the shipping item to the subscription, a subscription not been created for this row.', 'kwf-importer' ) );
				}
		...
		if ( ! empty( $tax_items ) ) {
			foreach ( $tax_items as $tax_item ) {
				...
				if ( ! empty( $tax_data['id'] ) ) {
					...
					$result['warning'][] = sprintf( esc_html__( 'Missing tax code or ID from column: %s', 'kwf-importer' ), $data['tax_items'] );
				}
...
new TPRM_importer_WooCommerceSubscriptions();