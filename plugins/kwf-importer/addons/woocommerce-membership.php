...
add_filter( 'TPRM_importer_restricted_fields', 'TPRM_importer_wm_restricted_fields', 10, 1 );
add_action( 'TPRM_importer_documentation_after_plugins_activated', 'TPRM_importer_wm_documentation_after_plugins_activated' );
add_action( 'post_TPRM_importer_import_single_user', 'TPRM_importer_wm_post_import_single_user', 10, 3 );

function TPRM_importer_wm_restricted_fields( $TPRM_importer_restricted_fields ){
	return array_merge( $TPRM_importer_restricted_fields, array( 'member_first_name', 'member_last_name', 'member_email', 'membership_plan_id', 'membership_plan_slug', 'membership_plan', 'membership_status', 'member_since', 'membership_expiration' ) );
}

function TPRM_importer_wm_documentation_after_plugins_activated(){
	?>
	<tr valign="top">
		<th scope="row"><?php _e( "WooCommerce Memberships is activated", 'kwf-importer' ); ?></th>
		<td>
			<ol>
				<li><strong><?php _e( "Import all membership information", 'kwf-importer' ); ?></strong>: <?php _e( "You can use the <strong>columns in the CSV format created by WooCommercer Membership</strong> in order to import data from this plugin.", 'kwf-importer' ); ?>. <a href="https://docs.woocommerce.com/document/woocommerce-memberships-import-and-export/"><?php _e( "Read more about columns and formats", 'kwf-importer' ); ?></a>.</li>
				<li><strong><?php _e( "Add users to membership plans", 'kwf-importer' ); ?></strong>: <?php _e( "In this case you will only have to use <strong>membership_plan_id</strong> column in order to associate a user to their membership plan", 'kwf-importer' ); ?>.</li>
			</ol>
		</td>
	</tr>
	<?php
...
function TPRM_importer_wm_post_import_single_user( $headers, $row, $user_id ){
	...
	if( $full_membership_import ){
		...
		TPRM_importer_vm_update_user_membership_meta( $user_membership, $action, $import_data );
...
function TPRM_importer_vm_update_user_membership_meta( WC_Memberships_User_Membership $user_membership, $action, array $data ) {
