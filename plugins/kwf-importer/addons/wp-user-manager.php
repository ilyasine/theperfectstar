...
class TPRM_importer_WP_User_Manager{
	function __construct(){
		add_filter( 'TPRM_importer_force_reset_password_edit_profile_url', array( $this, 'force_reset_password_edit_profile_url' ) );
        add_filter( 'TPRM_importer_force_reset_password_redirect_condition', array( $this, 'force_reset_password_redirect_condition' ), 10 , 1 );
    ...
    function force_reset_password_notice(){
        if ( get_user_meta( get_current_user_id(), 'TPRM_importer_force_reset_password', true ) ) {
            echo apply_filters( 'TPRM_importer_force_reset_password_message', __( '<span class="TPRM_importer_force_reset_password_message">Please change your password</span>', 'kwf-importer' ) );
        }
    }
    ...
    function maybe_force_reset_save_account_details(){
        ...
        if( isset( $_GET['password-updated'] ) && $_GET['password-updated'] == 'success' && isset( $_GET['tab'] ) && $_GET['tab'] == 'password' )
            delete_user_meta( get_current_user_id(), 'TPRM_importer_force_reset_password' );
    }
}
new TPRM_importer_WP_User_Manager();