...
class TPRM_importer_Groups{
	function __construct(){
		...
		add_action( 'post_TPRM_importer_import_single_user', array( $this, 'import_single_user_by_name' ), 11, 3 );
	}

	function restricted_fields( $TPRM_importer_restricted_fields ){
		return array_merge( $TPRM_importer_restricted_fields, array( 'group_id' ) );
	}
	
	function documentation(){
		?>
		<tr valign="top">
			<th scope="row"><?php _e( "Groups is activated", 'kwf-importer' ); ?></th>
			<td>
				<?php _e( "You can import user and assign them to the users groups using the next format", 'kwf-importer' ); ?>.
				<ul style="list-style:disc outside none; margin-left:2em;">
					<li><?php _e( "group_id as the column title", 'kwf-importer' ); ?></li>
					<li><?php _e( "The value of each cell will be the ID of the group that you want to assign to this user", 'kwf-importer' ); ?></li>
					<li><?php _e( "Another option is use group_name as the column title", 'kwf-importer' ); ?></li>
					<li><?php _e( "The value of each cell will be the name of the group that you want to assign to this user", 'kwf-importer' ); ?></li>
