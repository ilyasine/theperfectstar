...
class TPRM_importer_WooCommerce{
	private $all_virtual;
	...
	function __construct(){
		...
		add_action( 'post_TPRM_importer_import_single_user', array( $this, 'sync_wc_customer' ), 10, 4 );
		add_action( 'after_TPRM_importer_import_users', array( $this, 'clear_transients' ) );
		add_filter( 'TPRM_importer_import_email_body_before_wpautop', array( $this, 'include_overrides_email' ), 10, 5 );
		add_action( 'TPRM_importer_email_wildcards_list_elements', array( $this, 'new_wildcards_email' ) );
        add_filter( 'TPRM_importer_force_reset_password_edit_profile_url', array( $this, 'force_reset_password_edit_profile_url' ) );
        add_filter( 'TPRM_importer_force_reset_password_redirect_condition', array( $this, 'force_reset_password_redirect_condition' ) );
	...
	?>
		<tr valign="top">
			<th scope="row"><?php _e( "WooCommerce is activated", 'kwf-importer' ); ?></th>
			<td><?php _e( "You can use those labels if you want to set data adapted to the WooCommerce default user columns", 'kwf-importer' ); ?>
				<ol>
	...
	function include_overrides_email( $body, $headers, $data, $created, $user_id ){
		...
		$woocommerce_password_reset_url_link = wp_sprintf( '<a href="%s">%s</a>', $woocommerce_password_reset_url, __( 'Password reset link', 'kwf-importer' ) );
	...
	function new_wildcards_email(){
		?>
		<li>**woocommercelostpasswordurl** = <?php _e( 'WooCommerce lost password url', 'kwf-importer' ); ?></li>
		<li>**woocommercepasswordreseturl** = <?php _e( 'WooCommerce password reset url', 'kwf-importer' ); ?>
		<li>**woocommercepasswordreseturllink** = <?php _e( 'WooCommerce password reset url with HTML link', 'kwf-importer' ); ?>
		<?php
	}
    ...
    function force_reset_password_notice(){
        if ( get_user_meta( get_current_user_id(), 'TPRM_importer_force_reset_password', true ) && !wc_has_notice( apply_filters( 'TPRM_importer_force_reset_password_message', __( 'Please change your password', 'kwf-importer' ) ), 'error' ) ) {
            wc_add_notice( apply_filters( 'TPRM_importer_force_reset_password_message', __( 'Please change your password', 'kwf-importer' ) ), 'error' );
        }
    }
    ...
    function force_reset_save_account_details( $user_id ){
		...
		delete_user_meta( $user_id, 'TPRM_importer_force_reset_password' );
    }
}

new TPRM_importer_WooCommerce();