...
class TPRM_importer_ACF{
	function __construct(){
		add_filter( 'TPRM_importer_restricted_fields', array( $this, 'restricted_fields' ), 10, 1 );
		add_filter( 'TPRM_importer_not_meta_fields', array( $this, 'restricted_fields' ), 10, 1 );
	...
	function restricted_fields( $TPRM_importer_restricted_fields ){
		return array_merge( $TPRM_importer_restricted_fields, $this->get_user_fields_keys() );
	}
	...
	function documentation(){
		...
		?>
		<tr valign="top">
			<th scope="row"><?php _e( "Advaced Custom Fields is activated", 'kwf-importer' ); ?></th>
			<td>
				<?php _e( "You can import those fields, look at every group which fields you can import using the column names shown below.", 'kwf-importer' ); ?>.
				<ul style="list-style:disc outside none; margin-left:2em;">
				<?php foreach ( $this->get_user_fields() as $group => $fields ): ?>
					<li><?php _e( "Group name", 'kwf-importer' ); ?>: <em><?php echo $group; ?></em></li>
		...
		foreach ( $fields_positions as $pos => $key ) {
			if( $types[ $key ][ 'type' ] == 'relationship' ){
				...
				foreach ( $data as $it => $value ) {
					$data[ $it ] = is_numeric( $value ) ? $value : TPRM_importer_Helper::get_post_id_by_slug( $value );
				}
			}
