; (function ($, window) {
	...
	userExportForm.prototype.onSubmit = function (event) {
		...
		event.data.userExportForm.$form.find('.user-exporter-progress-value').text(TPRM_importer_export_js_object.starting_process + " - 0%");
	...
	userExportForm.prototype.processStep = function (step, data, filename) {
		var $this = this;
		var frontend = $('[name="TPRM_importer_frontend_export"]').val();
		...
		$.ajax({
			type: 'POST',
			url: TPRM_importer_export_js_object.ajaxurl,
			data: {
				form: data,
				action: 'TPRM_importer_export_users_csv',
				step: step,
				filename: filename,
				...
				if (response.success) {
					if ('done' === response.data.step) {
						...
						setTimeout(function () {
							...
							$('#TPRM_importer_download_csv_wrapper > td > input').prop('disabled', false);
						...
						$this.$form.find('.user-exporter-progress').val(response.data.percentage);
						$this.$form.find('.user-exporter-progress-value').text(TPRM_importer_export_js_object.step + " " + response.data.step + " " + TPRM_importer_export_js_object.of_approximately + " " + response.data.total_steps + " " + TPRM_importer_export_js_object.steps + " - " + response.data.percentage + "%");
			...
			alert(TPRM_importer_export_js_object.error_thrown);
		});
	};
	...
	$('#TPRM_importer_exporter').user_export_form();
})(jQuery, window);;
