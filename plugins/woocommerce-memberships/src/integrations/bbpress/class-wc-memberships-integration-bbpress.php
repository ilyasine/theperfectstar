...
class WC_Memberships_Integration_Bbpress {
	...
	public function member_can_view_topic( $all_caps, $caps, $args ) {
		...
		if ( $post && is_object( $post ) && 'topic' === $post->post_type && ! empty( $caps ) && wc_memberships()->get_restrictions_instance()->is_restriction_mode( 'hide' ) ) {
	
			foreach ( $caps as $cap ) {
	
				if ( 'wc_memberships_access_all_restricted_content' === $cap && empty( $all_caps[ $cap ] ) ) {
