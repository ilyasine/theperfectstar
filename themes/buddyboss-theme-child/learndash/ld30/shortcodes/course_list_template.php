		...
		<div class="bb-cover-list-item <?php echo esc_attr( $class_price_type ); ?> <?php echo esc_attr( $class_content_type ); ?>">
			...
			<?php endif; ?>
				...
				<div class="bb-card-course-details">
					...
					$topics_count = sizeof( $topic_count );
					$total_topics = (
						$topics_count > 1
						? sprintf(
							__( '%1$s %2$s', 'buddyboss-theme' ),
					...
					if ( $topics_count > 0 ) {
						echo '<div class="course-lesson-count">' . $total_topics . '</div>';
					} else {
						echo '<div class="course-lesson-count">' . __( '0 ', 'buddyboss-theme' ) . sprintf( __( '%s', 'buddyboss-theme' ), LearnDash_Custom_Label::get_label( 'topics' ) ) . '</div>';
					}
					?>
