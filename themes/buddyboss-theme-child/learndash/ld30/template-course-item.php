...
<li class="bb-course-item-wrap">
    ...
    <div class="bb-cover-list-item <?php echo $class; ?>">
        <div class="bb-course-cover">
            <a title="<?php the_title_attribute(); ?>" href="<?php the_permalink(); ?>" class="bb-cover-wrap">
				...
				if ( is_user_logged_in() && isset( $user_course_has_access ) && $user_course_has_access ) {
					...
					echo '<div class="ld-status ld-status-incomplete ld-third-background">' . __( 'Buy Now',
							'buddyboss-theme' ) . '</div>';
        ...
        <div class="bb-card-course-details <?php echo ( is_user_logged_in() && isset( $user_course_has_access ) && $user_course_has_access ) ? 'bb-card-course-details--hasAccess' : 'bb-card-course-details--noAccess'; ?>">
			...
			if ( $lessons_count > 0 ) {
				echo '<i class="bb-icons bb-icon-book"></i><span> ' . $total_lessons . '     </span>';
			} 
			...
			if ( $topics_count > 0 ) {
				echo '<i style="margin-left: 10px;" class="bb-icons bb-icon-text"></i><span> ' . $total_topics . '</span>';
			}
			
			$course_duration = get_post_meta( $course_id, 'course_length', true );
			
			if ( $course_duration ) {
				echo '<i style="margin-left: 10px;" class="bb-icons bb-icon-clock"></i><span> ' . $course_duration . '</span>';
			}			
