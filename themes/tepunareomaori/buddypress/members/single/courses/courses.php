...
if (! empty($filepath)) {
	wp_enqueue_script('learndash_template_Exam Sectionscript_js', str_replace(ABSPATH, '/', $filepath), array('jquery'), LEARNDASH_VERSION, true);
...
$defaults = array(
	...
	'per_page'           => false,
	'order'              => 'DESC', // student course order
	'orderby'            => 'date', // student course order
...
if (is_tprm_admin()) {
	...
	// Display select menu
	/*?>	
	<select name="courses_archive" id="courses_archive">
	<option value="<?php echo this_year_group(); ?>" selected><?php _e('My Archive', 'tprm-theme') ?></option> <!-- Default option -->
	<?php 
	foreach ($user_groups as $user_group) {
		$bp_group_id = get_post_meta($user_group, '_sync_group_id', true);
		if( this_year_group() != $user_group ) :
		?>
		<option value="<?php echo esc_attr($user_group); ?>"><?php echo get_the_title($user_group);?> <?php echo esc_html(groups_get_groupmeta( $bp_group_id, 'ecole_year')) ?></option>
		<?php 
		endif;
	}
	?>
...
<div id="bb-learndash_profile" class="<?php echo empty($user_courses) ? 'user-has-no-lessons' : ''; ?>">
	...
	<div id="learndash-content" class="learndash-course-list">
		<!-- style="flex:66.66%;padding: 15px; border-right: 1px solid #ddd;" -->
		<?php
				...
				<div class="grid-view bb-grid">
					<div id="course-dir-list" class="course-dir-list bs-dir-list">
						...
						if (! empty($user_courses)) :
							...
							$_post = $post;
						?>
							<ul class="bb-course-list bb-course-items list-view bb-list" aria-live="assertive" aria-relevant="all">
								<?php
						...
						/* if (!empty($user_groups)) {
							foreach ($user_groups as $user_group) {
								...
								echo '<h2>' . get_the_title($user_group) . ' Courses</h2>';

								if (!empty($group_courses)) {
									echo '<ul class="bb-course-list bb-course-items ' . esc_attr($class_grid_show . $class_list_show) . '" aria-live="assertive" aria-relevant="all">';
									...
									echo '<p>No courses found for this group.</p>';
								}
							}
						} else {
							echo '<p>No groups found for the user.</p>';
						} */
