...
$access_list            = learndash_get_course_meta_setting($post, 'course_access_list');
...
<li class="bb-course-item-wrap" data-course-id="<?php echo esc_attr($course_id) ?>" data-group-id="<?php echo esc_attr($ld_group_id) ?>" data-position="<?php echo esc_attr($course_position) ?>" data-security="<?php esc_attr_e($TPRM_ajax_nonce); ?>">
	...
	<div class="bb-cover-list-item <?php echo $class; ?>">
		...
		<div class="bb-card-course-details">
			...
			<div>
				...
				<?php
				if ($lessons_count > 0) {
					echo '<div class="course-lesson-count">' . $total_lessons . '</div></div>';
				} else {
					echo '<div class="course-lesson-count">' . sprintf(__('0 %s', 'buddyboss'), LearnDash_Custom_Label::get_label('lessons')) . '</div></div>';
				}
				...
				if (is_user_logged_in() && isset($user_course_has_access) && $user_course_has_access) { ?>
					...
					<div class="course-progress-wrap">
						...
						echo '<script>';
						echo 'var xyz = ' . json_encode($xyz) . ';'; // Convert PHP array to JavaScript object
						echo '</script>';
						?>
						<script>
							document.addEventListener("DOMContentLoaded", function() {
								if (xyz && xyz.post_id) {
									var elementId = 'ld-expand-' + xyz.post_id;
									var element = document.getElementById(elementId);
									if (element) {
										element.classList.add('highlight'); // Add a class to highlight the element
									}
								}
							});
						</script>
				...
				<?php } else { ?>
					<div class="bb-course-excerpt" style="flex: 60%;margin-top:35px;">
			...
			<?php  //do_action('course_final_quiz', $course_id) 
			?>
			<?php do_action('course_actions', $course_id) ?>
